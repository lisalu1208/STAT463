...
=======
As we can see from the coeffients of the variables in model chosen in Q2, **age** and **use** have positive signs, which indicates that the age and the contraceptive use increase the number of living children. Similarly, with the negative signs, **district14** and **urban** have negative effect on the number of the living children. 

To calculate the response scale, exp() function is used here.
```{r}
# age
...
```
The number here suggests that the contraceptive use is associated with an increase of 41.55% in living children rate among Bangladeshi women.

```{r}
# urbanY
urban_response <- (exp(-poisson_model1$coefficients['urbanY']) - 1) * 100; urban_response
```
This can be interpreted that living in the urban area is associated with a reduction of 19.51% in the rate of the number of living children among Bangladeshi women.
